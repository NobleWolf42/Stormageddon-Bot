    ...
    execute(message, args, client) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (serverID != null && serverConfig != undefined) {
                if (serverConfig.modMail.enable) {
                    for (const key of serverConfig.modMail.modList) {
                        ...
                        const embMsg = new EmbedBuilder()
                            .setTitle(`Mod Mail from: ${(yield client.guilds.fetch(serverID)).name}`)
                            .setColor('#0B6E29')
                ...
                return errorCustom(message, `The \`!setup\` command has not been run on \`${(yield client.guilds.fetch(serverID)).name}\` yet.`, this.name, client);
            }
            else {

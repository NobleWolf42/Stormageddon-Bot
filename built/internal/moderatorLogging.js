...
//#region Imports
import { AuditLogEvent, Collection, EmbedBuilder, Events } from 'discord.js';
...
function logAdminUpdate(client) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        client.on(Events.ChannelUpdate, (oldChannel, newChannel) => __awaiter(this, void 0, void 0, function* () {
            if (oldChannel.isDMBased() || newChannel.isDMBased()) {
                return;
            }
            const serverConfig = yield MongooseServerConfig.findById(oldChannel.guild.id);
            // check the server config and see if they have logging turned on
            // is the bot setup on the server?
            if (serverConfig.setupNeeded) {
                return;
            ...
            if (oldChannel.permissionOverwrites != newChannel.permissionOverwrites) {
                for (const key of oldChannel.permissionOverwrites.cache.keys()) {
                    if (!oldChannel.permissionOverwrites.resolve(key) ||
                        ...
                        let roleName = yield oldChannel.guild.roles.fetch(key);
                        if (roleName == null) {
                            roleName = yield client.users.fetch(key);
                        }
 ...
 * @param client - Discord.js Client
 */
function logUserUpdate(client) {
    return __awaiter(this, void 0, void 0, function* () {
        //#region User Update
        client.on(Events.GuildMemberUpdate, (oldMember, newMember) => __awaiter(this, void 0, void 0, function* () {
            const serverConfig = yield MongooseServerConfig.findById(oldMember.guild.id);
            // check the server config and see if they have logging turned on
            // is the bot setup on the server?
            if (serverConfig.setupNeeded) {
                return;
            ...
            let auditLogFetch = yield oldMember.guild.fetchAuditLogs({
                limit: 1,
        ...
        //#endregion
        //#region User Leaving
        client.on(Events.GuildMemberRemove, (member) => __awaiter(this, void 0, void 0, function* () {
            const serverConfig = yield MongooseServerConfig.findById(member.guild.id);
            // check the server config and see if they have logging turned on
            // is the bot setup on the server?
            if (serverConfig.setupNeeded) {
                return;

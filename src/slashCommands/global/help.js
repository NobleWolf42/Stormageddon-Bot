...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .addStringOption((option) =>
            option
                .setName('helppage')
                .setDescription(
                    'Select a help page (admin/direct/fun/gaming/help/music).'
                )
                .setRequired(false)
                .setChoices(
    ...
    execute(client, interaction, distube) {
        ...
        if (process.env.devIDs.includes(interaction.author.id)) {
            ...
            commands.forEach((cmd) => {
                if (
                    !commandClasses.includes(capitalize(cmd.class)) &&
        ...
        if (
            interaction.options.getString('helppage') == undefined ||
            ...
            commandClasses.includes(
                capitalize(interaction.options.getString('helppage'))
            ) ||
            ...
            switch (interaction.options.getString('helppage')) {
                ...
                case 'developer':
                    if (process.env.devIDs.includes(interaction.author.id)) {
                        ...
                        return warnCustom(
                            interaction,
                            `The **${args[0]}** page you requested does not exit. Please select from these pages: \`${makeCommandPageList(commandClasses)}\``,
                            module.name
                        );
                    }

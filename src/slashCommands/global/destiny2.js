...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .addSubcommand((subcommand) =>
            subcommand
                .setName('clan')
                .setDescription(
                    "Displays Destiny 2 clan's bio, avatar, motto, and founder"
                )
                .addStringOption((option) =>
                    option
                        .setName('clanname')
                        .setDescription(
                            'Name of clan you want to see information on.'
                        )
                        .setRequired(true)
                )
        ),
    execute(client, interaction, distube) {
        if (interaction.options.getSubcommand() == 'clan') {
            ...
            warnCustom(
                interaction,
                'You did not use the command correctly, please try again.',
                module.name,
                client
            );
        }

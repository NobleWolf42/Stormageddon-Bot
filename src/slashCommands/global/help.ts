...
const helpSlashCommand: SlashCommand = {
    ...
    async execute(_client, interaction, _distube, collections) {
        //#region Escape Logic
        ...
        const helpMessageCommands: Command[] = [];
        ...
        if (interaction.options.getString('helppage') == undefined || interaction.options.getString('helppage') == 'help') {
            ...
            !commandClasses.includes(capitalize(interaction.options.getString('helppage'))) ||
            ...
            warnCustom(
                interaction,
                `The **${interaction.options.getString('helppage')}** page you requested does not exit. Please select from these pages: \`${makeCommandPageList(commandClasses)}\``,
                helpSlashCommand.data.name
            );
            return;
        }
 ...
 */
function makeHelpMsg(interaction: InteractionWithChanges, title: string, helpMessageCommands: Command[], commandClasses: string[], adminTF: boolean) {
    let helpMsg = '';
    commandClasses.sort();

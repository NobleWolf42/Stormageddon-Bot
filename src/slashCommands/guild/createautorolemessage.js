...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .setDescription(
            'Creates the reactions message for auto role assignment.'
        )
    ...
    async execute(client, interaction, distube) {
        ...
        if (
            ...
            throw new Error(
                'Roles list and reactions list are not the same length! Please double check this in the config[serverID].js file'
            );
        }
        ...
        for (var i = 0; i < Math.ceil(fieldsOut.length / maxReactions); i++) {
            ...
            channel.send({ embeds: [embMsg] }).then(async (m) => {
                var maxEmoji = 0;
                if (

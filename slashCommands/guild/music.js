...
module.exports = {
    ...
    async execute(client, interaction, distube) {
        ...
        if (serverConfig[interaction.guildId].music.textChannel != channel.name) {
            return warnWrongChannel(interaction, serverConfig[interaction.guildId].music.textChannel, module.name, client);
        }
        ...
        switch (interaction.options.getSubcommand()) {
                ...
                if (!queue) {
                    ...
                    switch (loopMode) {
                        case "song":
                            ...
                            embedCustom(interaction, `Loop On - Song`, "#0E4CB0", "Music set to loop song.", { text: `Requested by ${interaction.user.username}`, iconURL: null }, null, [], null, null);
                        break;
                            
                        case "queue":
                            queue.setRepeatMode(2);
                            embedCustom(interaction, `Loop On - Queue`, "#0E4CB0", "Music set to loop queue.", { text: `Requested by ${interaction.user.username}`, iconURL: null }, null, [], null, null);
                        break;
            ...
            case "remove":
                ...
                if (!queue) {
                    ...
                    queue.skip().then(() => {
                        embedCustom(interaction, "Removed", "#0000FF", `Removed [\`${son.name}\`](${song.url}) from the queue.`, { text: `Requested by ${interaction.user.username}`, iconURL: null }, null, [], null, null);
                    });
                } else {
            ...
            case "skipto":
                ...
                if (!queue) {
                    ...
                    queue.skip().then(s => {
                        embedCustom(interaction, "Skipped", "#0000FF", `Skipped to [\`${s.name}\`](${s.url}).`, { text: `Requested by ${interaction.user.username}`, iconURL: null }, null, [], null, null);
                    });
                }
            break;
            //#endregion
            ...
            case "volume":
                ...
                if (!queue) {
                    ...
                    embedCustom(interaction, "Volume", "#0000FF", `Volume is currently ${queue.volume}%.`, { text: `Requested by ${interaction.user.username}`, iconURL: null }, null, [], null, null);
                } else {

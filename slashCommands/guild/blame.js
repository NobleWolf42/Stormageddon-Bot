    ...
    data: new SlashCommandBuilder()
        ...
        if (serverConfig[serverID].blame.enable) {
            ...
            if (serverConfig[serverID].blame.rotateList.length > 0) {
                blameList.push(serverConfig[serverID].blame.rotateList[Math.floor(Date.now() / 604800000) - (Math.floor(Math.floor(Date.now() / 604800000) / serverConfig[serverID].blame.rotateList.length) * serverConfig[serverID].blame.rotateList.length)]);
